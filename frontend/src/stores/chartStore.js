...
export function calculateBarChart(compdocs) {
  ...
  for (let i = 0; i < compdocs.length; i++) {
    ...
    for (let j = 0; j < statusFlowLength; j++) {
      ...
      if (
        flow.status == 'to_be_updated' ||
        (SHOW_DELAYED_COMPDOCS
          ? flow.status == 'delayed'
          : statusFlowLength == 1 && flow.status == 'to_be_issued' && diff > 0)
      ) {
        counter['ubm'] += absDiff
...
export function calculatePieChart(compdocs) {
  ...
  compdocs.forEach((compdoc) => {
    if (SHOW_DELAYED_COMPDOCS) {
      ...
      if (
        compdoc.status_flow.length == 1 &&
        compdoc.status_flow[0].status == 'to_be_issued' &&
        (getDaysDifference(getTodayEUFormat(), compdoc?.ubm_target_date) || 0) > 0
      ) {
        counter['delayed']++
      } else {
        counter[compdoc.status]++
...
export function calculateLineChart(compdocs) {
  ...
  let scheduled = Object.entries(counterScheduled)
    .map(([key, value]) => ({ x: key, y: value }))
    .sort(sortByDate)
  let actual = Object.entries(counterActual)
    .map(([key, value]) => ({ x: key, y: value }))
    .sort(sortByDate)
  ...
  let result = {
    ...
    today: [today],
    lastScheduled: {
      x: scheduled[scheduledClosestIndex]?.x,
      y: scheduled[scheduledClosestIndex]?.y
    },
