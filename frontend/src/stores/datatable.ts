...
export function getDateFilterMenuFunc(attrib: string, onApplyEvent: any, onCleanEvent: any) {
  const dateFilterMenuFunc = (actions: any) => {
    if (dateFilterContainer.value[attrib] == null)
      ...
      },
      {
        default: () => [
          ...
          h(
            NButton,
            {
              type: dateFilterContainer.value[attrib]['filter']
                ? 'error'
                : !dateFilterContainer.value[attrib]?.date ||
                    !dateFilterContainer.value[attrib]?.type
                  ? 'default'
                  : 'success',
              disabled:
                !dateFilterContainer.value[attrib]?.date ||
...
export function getArrayFilterMenuFunc(attrib: string, onApplyEvent: any, onCleanEvent: any) {
  const arrayFilterMenuFunc = (actions: any) => {
    if (arrayFilterContainer.value[attrib] == null)
      ...
      },
      {
        default: () => [
          ...
          h(
            NButton,
            {
              type: arrayFilterContainer.value[attrib]['filter']
                ? 'error'
                : !arrayFilterContainer.value[attrib]?.values.length
                  ? 'default'
                  : 'success',
...
export function getArrayFilterFunc(attrib: string) {
  const arrayFilterFunc = (value: string[], row: Record<string, any>) => {
    ...
    if (getType(value) == 'array')
      return value.length > 0 ? value.some((item) => ~row[attrib].indexOf(item)) : true
