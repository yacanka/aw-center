    ...
    <n-form ref="formRef" :model="panel" :rules="rules">
      <n-grid responsive="self" item-responsive :x-gap="12" :cols="12">
        <n-form-item-gi span="12" path="project" label="Project">
          <n-select
            v-model:value="panel.project"
            :options="projectOptions"
            placeholder="Select Project"
            disabled
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 640:5" path="name" label="Name">
          <n-input
            v-model:value="panel.name"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 640:5" path="discipline" label="Discipline">
          <n-input
            v-model:value="panel.discipline"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 640:2" path="ata" label="ATA Chapter">
          <n-input
            v-model:value="panel.ata"
            placeholder="XX-XX"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
...
import { IPanel, IProject, type OrganizationOption } from '@/features/organization/models/orgs'
...
async function addDatabase() {
  ...
  try {
    await orgsStore.createPanel(panel.value)
    closeModal()
  } catch (err) {
    console.error(err)
  }
...
async function updateDatabase() {
  ...
  try {
    await orgsStore.updatePanel(panel.value.id, panel.value)
    closeModal()
  } catch (err) {
    console.error(err)
  }
