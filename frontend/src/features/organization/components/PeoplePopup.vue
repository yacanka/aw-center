  ...
  >
    ...
    <n-form ref="formRef" :model="person" :rules="rules">
      <n-grid responsive="self" item-responsive :x-gap="12" :cols="12">
        <n-form-item-gi span="0:12 640:3" path="person_id" label="ID">
          <n-input
        ...
        <n-form-item-gi span="0:12 640:4" path="name" label="Name">
          <n-input
        ...
        <n-form-item-gi span="0:12 640:5" path="email" label="Email">
          <n-input
            type="email"
...
async function addDatabase() {
  ...
  await orgsStore.createPerson(person.value)
  closeModal()
}

async function updateDatabase() {
  if (!(await validateForm(formRef.value))) return
  if (person.value.id === undefined) throw new Error('Cannot update a person without an ID.')
  await orgsStore.updatePerson(person.value.id, person.value)
  closeModal()
}
