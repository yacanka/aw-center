  ...
  >
    ...
    <n-form ref="formRef" :model="ddf" :rules="rules">
      <n-grid responsive="self" item-responsive :x-gap="12" :cols="12">
        <n-form-item-gi span="0:12 760:3" path="project" label="Project">
          <n-input
            v-model:value="ddf.project"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 760:4" path="doc_name" label="Doc Name">
          <n-input
            v-model:value="ddf.doc_name"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>

        <n-form-item-gi span="0:12 760:2" path="doc_no" label="Doc No">
          <n-input
            v-model:value="ddf.doc_no"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 760:1" path="doc_issue" label="Doc Issue">
          <n-input
            v-model:value="ddf.doc_issue"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 760:2" path="date" label="Date">
          <n-date-picker
            v-model:formatted-value="ddf.date"
            type="date"
            format="dd.MM.yyyy"
            :firstDayOfWeek="0"
            :readonly="popupMode == 'view'"
        ...
        <n-form-item-gi span="0:12 760:3" path="commentor" label="Commentor">
          <n-input
            v-model:value="ddf.commentor"
            @keydown.enter.prevent
            :readonly="popupMode == 'view'"
          />
        </n-form-item-gi>
        <n-form-item-gi span="0:12 760:9" path="path" label="Reference Path">
          <n-input
            v-model:value="ddf.path"
            placeholder="Path"
            :readonly="popupMode == 'view'"
            @click="popupMode == 'view' ? copyToClipboard() : null"
            @keydown.enter.prevent
          />
        </n-form-item-gi>
      </n-grid>
      <n-collapse @item-header-click="handleItemHeaderClick">
        <n-collapse-item title="Comments" name="comments">
          <n-dynamic-input
            ...
            "
          >
            <template #create-button-default> Add Comment </template>
            <template #default="{ value }">
              <n-grid responsive="self" item-responsive cols="13" x-gap="16">
                <n-grid-item span="0:13 760:1">
                  <n-input
                ...
                <n-grid-item span="0:13 760:2">
                  <n-input
                    v-model:value="value[1]"
                    placeholder="Relevant Section"
                    :readonly="popupMode == 'view'"
                  />
                </n-grid-item>

                <n-grid-item span="0:13 760:5">
                  <n-input
                    type="textarea"
...
const ddfStore = useDdfController()
...
async function addDatabase() {
  ...
  await ddfStore.createDdf(ddf.value)
}

async function updateDatabase() {
  if (!(await validateForm(formRef.value))) return
  if (ddf.value.id === undefined) throw new Error('Cannot update a DDF without an ID.')
  await ddfStore.updateDdf(ddf.value.id, ddf.value)
  closeModal()
}
