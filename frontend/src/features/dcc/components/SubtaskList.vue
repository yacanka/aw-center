...
import NInputWidth from '@/shared/components/NInputWidth.vue'
...
const saveButton = ref<{ type: 'default' | 'warning' }>({ type: 'default' })
...
async function saveAllList() {
  if (saving.value) return
  saving.value = true
  const savedVersion = editVersion
  try {
    await session.updatePreference({ jira_list: JSON.parse(JSON.stringify(subtaskLists.value)) })
    if (savedVersion === editVersion) saveButton.value.type = 'default'
  } catch {
    // The session store reports the error; retain the unsaved edits for retry.
  } finally {
    saving.value = false
  }
}
...
function applyBulkValue(item: ISubtaskItem, key: string, value: JiraFieldValue) {
  ...
  else ensureFields(item)[key] = Array.isArray(value) ? [...value] : value
}
...
function normalizeLists(value: unknown): ISubtaskListItem[] {
  ...
  // Preferences retain the main-branch shape, but editing must not mutate session state.
  return JSON.parse(JSON.stringify(value)).filter(
    (item: ISubtaskListItem | null) =>
      item && typeof item.title === 'string' && Array.isArray(item.list)
  )
}

onMounted(() => {
  subtaskLists.value = normalizeLists(session.getPreferences.jira_list)
  if (subtaskLists.value.length == 0) handleTabAdd()
  const storedTab = parseInt(localStorage.getItem('jira>activetab>sglist') || '0')
  setActiveList(storedTab >= 0 && storedTab < subtaskLists.value.length ? storedTab : 0)
})
</script>
