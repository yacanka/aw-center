...
const columns: DataTableColumns<ManagedInvitation> = [
  ...
  },
  {
    ...
    key: 'created_by',
    width: 140,
    render: (row) => row.created_by || 'Deleted user'
  },
  { title: 'Created', key: 'created_at', width: 150, render: (row) => formatDate(row.created_at) },
  { title: 'Expires', key: 'expires_at', width: 150, render: (row) => formatDate(row.expires_at) },
  { title: 'Used by', key: 'used_by', width: 120, render: (row) => row.used_by || '—' },
  {
    title: '',
    key: 'actions',
    width: 90,
