...
export async function createUserInvitation(
  ...
  const response = await axios.post<CreatedInvitation>('users/invitations/', {
    email,
    group_ids: groupIds
  })
...
export async function listUserInvitations(filters: InvitationFilters): Promise<InvitationPage> {
  const response = await axios.get<InvitationPage>('users/invitations/', { params: filters })
  return response.data
}
...
export async function revokeUserInvitation(invitationId: number): Promise<ManagedInvitation> {
  const response = await axios.delete<ManagedInvitation>(`users/invitations/${invitationId}/`)
  return response.data
}
...
export async function inspectUserInvitation(token: string): Promise<InvitationContext> {
  const response = await axios.post<InvitationContext>('users/invitations/inspect/', { token })
  return response.data
}
...
export async function acceptUserInvitation(account: InvitationAccount): Promise<void> {
  await axios.post('users/invitations/accept/', account)
}
