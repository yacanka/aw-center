    ...
    <n-drawer-content :title="job?.title || 'Job details'" closable>
      <n-spin :show="loading">
        <n-space v-if="job" vertical size="large">
          <n-descriptions :column="isNarrow ? 1 : 2" bordered size="small">
...
import JiraIssueDraftPanel from '@/features/jobs/components/JiraIssueDraftPanel.vue'
import { isFailedJobStatus, type Job, type JobStatus } from '@/features/jobs/api/jobs'
...
function eventType(status: JobStatus): 'default' | 'success' | 'error' | 'warning' | 'info' {
  ...
  if (isFailedJobStatus(status)) return 'error'
