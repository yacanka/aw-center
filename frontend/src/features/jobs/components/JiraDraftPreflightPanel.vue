  ...
  <n-card title="JIRA readiness" size="small" embedded>
    <n-space vertical>
      <n-alert type="info" :bordered="false">
        Uses the opaque JIRA connection established from the JIRA workspace. Credentials are never
        copied into this draft or browser state.
      </n-alert>
      <n-alert v-if="!connected" type="warning" :bordered="false">
        Connect JIRA from the JIRA workspace before checking requirements.
      </n-alert>
      <n-alert v-if="dirty" type="warning" :bordered="false">
        Save the current draft changes before checking JIRA requirements.
      </n-alert>
      <n-button secondary :loading="busy" :disabled="!connected || dirty" @click="emit('check')">
...
import type {
  JiraIssueDraftPreflight,
  JiraPreflightIdentity
