...
import type { Job, JobStatus } from '@/features/jobs/api/jobs'
...
export async function createWorkflowRun(
  ...
  const response = await axios.post<WorkflowRun>('workflows/', formData, {
...
export async function cancelWorkflowRun(workflowId: string): Promise<WorkflowRun> {
  const response = await axios.post<WorkflowRun>(`workflows/${workflowId}/cancel/`)
  return response.data
}
