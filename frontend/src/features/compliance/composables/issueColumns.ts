...
import { searchDocproof } from '@/features/integrations/api/docproof'
  ...
  const columns: DataTableColumns<ICompDoc> = [
    {
      ...
      align: 'center',
      render: (row) => issueCell(row.cover_page_no, row.cover_page_issue, issueValues.value)
    },
    {
      key: 'tech_doc_no',
    ...
    },
    {
      ...
      render: (row) =>
        h(NSpace, { vertical: true }, () => [
          issueCell(row.tech_doc_no, row.tech_doc_issue, issueValues.value),
          issueCell(row.tech_doc_no_2, row.tech_doc_issue_2, issueValues.value)
        ])
    }
  ]
...
function issueCell(documentNumber: unknown, expectedIssue: unknown, values: IssueValues) {
  ...
  return lookupTrigger(number, expectedIssue, values)
}

function lookupTrigger(number: string, expectedIssue: unknown, values: IssueValues) {
  return h(
    NText,
    {
...
function lookupIssue(number: string, values: IssueValues) {
  values[number] = null
  searchDocproof(number)
