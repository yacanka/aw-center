<template>
  <n-card title="Word Translator">
    <n-space
      ...
      justify="center"
      item-style="minWidth: 200px; width: 500px"
    >
      <n-upload
        ref="uploadForm"
        :show-file-list="true"
      ...
      >
        <n-upload-dragger>
          ...
          <n-p depth="3" style="margin: 8px 0 0 0"> Word file to be translated </n-p>
        </n-upload-dragger>
      </n-upload>
    </n-space>
    <n-form ref="form" :model="parameters" style="margin-top: 18px">
      <n-grid cols="12" x-gap="18">
        <n-form-item-gi path="translate_type" label="Translate Type" span="3" offset="4">
          <n-select
            v-model:value="parameters.translate_type"
        ...
        <n-form-item-gi span="1">
          <n-button
            @click="translate"
            :focusable="false"
            :disabled="file.length == 0 || !parameters.translate_type"
            style="width: 100%"
          >
            Translate
          </n-button>
...
function translate() {
  ...
  axios
    .post(`${axios.defaults.baseURL}/word/create_queue/`, formData)
    .then((res) => {
