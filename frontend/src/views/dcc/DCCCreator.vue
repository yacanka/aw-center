...
const canSubmit = computed(() => Boolean(dccStore.getSessionId.trim() && generator.url.trim()))
...
async function previewDcc(): Promise<void> {
  ...
  errorMessage.value = ''
  try {
    setCurrentJob(await previewDccDocumentJob(dccStore.getSessionId, generator.url))
