      ...
      if (!syncStatus.value.isFirstSynced) {
        if (row.active) {
          ...
          if (issueStats.value[row.issue]) {
            summary = !issueStats.value[row.issue].fields.subtasks.some(
              (item: any) => item.fields.status.name === 'Open'
            )
          }
          if (summary == null) {
            return h(
...
<template>
  ...
  <UploadForm
    ref="uploadPopup"
    :uploadUrl="axios.defaults.baseURL + '/dcc/upload/'"
