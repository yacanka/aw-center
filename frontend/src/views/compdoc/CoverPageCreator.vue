  ...
  <n-flex justify="center">
    <n-card title="Cover Page Creator from Excel" style="width: 90%">
      <n-grid cols="6" x-gap="12" y-gap="18">
        ...
        <n-grid-item span="6">
          <n-h4 style="margin: 0; text-align: center"> Required Columns </n-h4>
        </n-grid-item>
        <n-grid-item span="6">
...
function handleUploadReq() {
  ...
  axios
    .post(`${axios.defaults.baseURL}/excel/create_queue/`, formData)
    .then((res) => {
      const eventSource = createAuthenticatedEventSource(
        `/excel/excel_to_cover_pages_stream/${res.data}`
      )
