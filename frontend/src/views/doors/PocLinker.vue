<template>
  <n-flex justify="center">
    <n-card title="Requirement PoC Linker" style="width: 80%">
      <n-p> Groups PoC requirements and automatically links them across modules. </n-p>
      <n-grid cols="12" x-gap="12" y-gap="18">
        <n-grid-item span="12">
          <n-input
            v-model:value="linker.ref_module_name"
            type="text"
            placeholder="Reference Module Name"
            @keydown.enter.prevent
          />
        </n-grid-item>
        <n-grid-item span="12">
          <n-input
            v-model:value="linker.target_module_name"
            type="text"
        ...
        <n-grid-item span="12">
          <n-input
            v-model:value="linker.link_module_name"
            type="text"
            placeholder="Link Module Name"
            @keydown.enter.prevent
          />
        </n-grid-item>
        <n-grid-item span="4">
          <n-input
            v-model:value="linker.ref_attr_poc"
            type="text"
            placeholder="Reference Module Attribute (PoC List)"
            @keydown.enter.prevent
          />
        </n-grid-item>
        <n-grid-item span="4">
          <n-input
            v-model:value="linker.ref_attr_req"
            type="text"
            placeholder="Reference Module Attribute (Requirement)"
            @keydown.enter.prevent
          />
        </n-grid-item>
        <n-grid-item span="4">
          <n-input
            v-model:value="linker.target_attr_poc"
            type="text"
            placeholder="Target Module Attribute (PoC Info)"
            @keydown.enter.prevent
          />
        </n-grid-item>
        <n-grid-item span="3">
          <n-input-number
            :value="parseInt(linker.start_index)"
            min="0"
        ...
        <n-grid-item span="3">
          <n-input-number
            :value="parseInt(linker.text_length)"
            min="-1"
        ...
        <n-grid-item span="2" offset="0">
          <n-card size="small" title="Link Mode:">
            <template #header-extra>
              <n-switch
                size="small"
                v-model:value="linker.activeness"
                checked-value="true"
        ...
        <n-grid-item v-if="linker.activeness == 'true'" span="3">
          <n-card size="small" title="Link Direction:">
            <template #header-extra>
              <n-switch
                size="small"
                v-model:value="linker.direction"
                :theme-overrides="neutralSwitchTheme"
                checked-value="ref2tar"
                unchecked-value="tar2ref"
              >
                <template #checked> Reference → Target </template>
                <template #unchecked> Target → Reference </template>
              </n-switch>
            </template>
          </n-card>
        </n-grid-item>
          ...
          <n-card size="small" style="height: 100%">
            {{
              linker.activeness == 'true'
                ? modeDescriptions[0] +
                  ' Reference module will have ' +
                  (linker.direction == 'tar2ref' ? '"In"' : '"Out"') +
                  ' type link.'
                : modeDescriptions[1]
            }}
          </n-card>
