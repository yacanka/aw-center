<template>
  <n-space vertical justify="space-around">
    <n-space justify="space-around">
      ...
      <n-card v-if="jiraInfo.customfield_45000" title="ECD No">{{
        jiraInfo.customfield_45000
      }}</n-card>
      <n-card v-if="jiraInfo.customfield_45002" title="DCC Name">{{
        jiraInfo.customfield_45002
      }}</n-card>

      <n-card
        v-if="jiraInfo.ecd_path != undefined"
        title="ECD Document"
        :style="jiraInfo.ecd_path ? 'color: green' : 'color: orange'"
        >{{ jiraInfo.ecd_path ? 'Exist' : 'Not Exist' }}</n-card
      >
      <n-card
        v-if="jiraInfo.dcc_unsigned_path != undefined"
        title="Unsigned DCC"
        :style="jiraInfo.dcc_unsigned_path ? 'color: green' : 'color: orange'"
        >{{ jiraInfo.dcc_unsigned_path ? 'Exist' : 'Not Exist' }}</n-card
      >
      <n-card
        v-if="jiraInfo.dcc_signed_path != undefined"
        title="Signed DCC"
        :style="jiraInfo.dcc_signed_path ? 'color: green' : 'color: orange'"
        >{{ jiraInfo.dcc_signed_path ? 'Exist' : 'Not Exist' }}</n-card
      >

      <n-card
        v-if="jiraInfo.generalStatus != undefined"
        title="Subtasks Status"
        :style="jiraInfo.generalStatus ? 'color: green' : 'color: orange'"
      >
        {{
          jiraInfo.generalStatus ? 'Subtasks are fully finalized' : 'Outstanding subtasks remain'
        }}</n-card
      >
    </n-space>
    <n-space justify="space-around">
      <n-space v-if="jiraInfo.subtasks" v-for="(item, index) in jiraInfo.subtasks" :key="index">
        ...
        <n-card
          ...
          :style="
            jiraInfo.subtasks[index].fields.status.name == 'Closed'
              ? 'color: green'
              : 'color: orange'
          "
...
onMounted(() => {
  ...
  jiraInfo.value.generalStatus = !jiraInfo.value.subtasks.some(
    (item) => item.fields.status.name === 'Open'
  )
})
</script>
