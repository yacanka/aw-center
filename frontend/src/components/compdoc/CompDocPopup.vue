<template>
  ...
  >
      ...
      <n-grid :x-gap="12" :cols="48">
        <n-form-item-gi span="12" path="panel" label="Panel">
          <n-select
            ...
            :status="originalCompdoc.panel != compdoc.panel ? 'warning' : ''"
          />
        </n-form-item-gi>
        <n-form-item-gi span="4" path="ata" label="Ata">
          <n-select
            ...
            :disabled="popupMode == 'view'"
            :status="originalCompdoc.ata != compdoc.ata ? 'warning' : ''"
          />
        </n-form-item-gi>
        ...
        <n-form-item-gi span="12" path="signature_panel" label="Signature Panel">
          <n-select
            ...
            :disabled="popupMode == 'view'"
            :status="
              !checkArrayEquals(originalCompdoc.signature_panel, compdoc.signature_panel)
                ? 'warning'
                : ''
            "
          />
        ...
        <n-form-item-gi span="3" path="delivered_tech_doc_issue" label="Delivered Issue">
          ...
          <n-input
            ...
            :readonly="popupMode == 'view'"
            :status="
              originalCompdoc.delivered_tech_doc_issue != compdoc.delivered_tech_doc_issue
                ? 'warning'
                : ''
            "
          />
        ...
        <n-form-item-gi
          v-if="hasExtraFields"
          span="3"
          path="delivered_tech_doc_issue_2"
          label="Delivered Issue 2"
        >
          <template #label>
          ...
          <n-input
            ...
            :readonly="popupMode == 'view'"
            :status="
              originalCompdoc.delivered_tech_doc_issue_2 != compdoc.delivered_tech_doc_issue_2
                ? 'warning'
                : ''
            "
          />
        ...
        <n-form-item-gi span="4" path="cat" label="Cat">
          <n-select
            ...
            clearable
            :disabled="popupMode == 'view'"
            :status="originalCompdoc.cat != compdoc.cat ? 'warning' : ''"
          />
        </n-form-item-gi>
        <n-form-item-gi span="4" path="moc" label="MoC">
          <n-select
            ...
            clearable
            :disabled="popupMode == 'view'"
            :status="originalCompdoc.moc != compdoc.moc ? 'warning' : ''"
          />
        </n-form-item-gi>
        <n-form-item-gi span="8" path="mom_no" label="Mom No" :offset="1">
          <n-input
            ...
            :readonly="popupMode == 'view'"
            :status="originalCompdoc.mom_no != compdoc.mom_no ? 'warning' : ''"
          />
        </n-form-item-gi>
        ...
        <n-collapse-item title="History" name="history">
          <n-scrollbar style="max-width: 60%">
            <n-timeline horizontal>
              <n-timeline-item
                :type="
                  hobj.history_type == 'Created'
                    ? 'success'
                    : hobj.history_type == 'Changed'
                      ? 'warning'
                      : 'default'
                "
                :title="hobj.history_user"
                :time="hobj.history_date"
                v-for="(hobj, index) in compdoc.history"
                :key="index"
              />
              <n-text v-if="compdoc.history?.length == 0"> No historical records. </n-text>
            </n-timeline>
          </n-scrollbar>
        </n-collapse-item>
      </n-collapse>
    ...
    <template #action>
      ...
      <n-button v-else-if="popupMode == 'update' && canEdit" type="warning" @click="updateDatabase"
        >Update</n-button
      >
    </template>
  </n-modal>
