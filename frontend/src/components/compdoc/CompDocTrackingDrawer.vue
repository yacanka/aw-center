...
import './CompDocTrackingDrawer.css'
...
<template>
  ...
  >
    <n-drawer-content closable :native-scrollbar="false">
      <template #header>
        <n-space vertical :size="4" class="tracking-heading">
          <n-text depth="3">{{ project.toUpperCase() }} · TRACKING & ALERTS</n-text>
          <n-ellipsis :line-clamp="1">{{ document?.name }}</n-ellipsis>
        </n-space>
      </template>
