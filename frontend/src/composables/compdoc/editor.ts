...
import type { ICompDoc, IHistory } from '@/models/compdocs'
...
const rules: FormRules = {
  ...
  ata: [{ required: true, trigger: 'blur' }],
...
export function useCompDocEditor(canEdit: Ref<boolean>) {
  ...
  async function reloadBaseline(documentId: string): Promise<void> {
    const latest = await window.$compdocStore.fetchCompdoc(documentId)
    originalCompdoc.value = JSON.parse(JSON.stringify(latest)) as ICompDoc
    compdoc.value.source_history_id = latest.source_history_id
