...
from integrations.teamcenter.config import AuthMode, TeamcenterClientConfig
...
from integrations.teamcenter.protocol import make_envelope, raise_for_partial_errors
from integrations.teamcenter.response import read_bounded_content
from integrations.teamcenter.services import validate_transport_security
from integrations.teamcenter.transport import TeamcenterTransport
...
class TeamcenterProtocolTests(SimpleTestCase):
    ...
    def test_configuration_rejects_credential_and_query_bearing_urls(self):
        for url in (
            "https://user:secret@teamcenter.example/tc",
            "https://teamcenter.example/tc?redirect=https://attacker.test",
            "https:///missing-host",
        ):
            with self.subTest(url=url), self.assertRaises(TeamcenterConfigurationError):
                TeamcenterClientConfig(base_url=url, username="user", password="password")
...
class TeamcenterTransportTests(SimpleTestCase):
    ...
    def test_transport_disables_redirects(self):
        ...
        transport.get_bootstrap_response()
...
class TeamcenterApiTests(TestCase):
    ...
    @patch("integrations.teamcenter.views.execute_with_client")
    ...
    def test_property_updates_require_administrator(self):
        ...
        response = self.client.post(
            reverse("teamcenter_property_update_job"),
            {},
            format="json",
            HTTP_IDEMPOTENCY_KEY="teamcenter-test-1",
        )
...
class TeamcenterJobTests(JobTestCase):
    """Verify durable property update creation and uncertain-write handling."""
    ...
    @patch("integrations.teamcenter.job_executor.execute_with_client")
    def test_property_update_is_idempotent_and_runs_only_in_worker(self, execute_client):
        ...
        first = self.client.post(
            ...
            payload,
            format="json",
            HTTP_IDEMPOTENCY_KEY="teamcenter-write-1",
        )
        ...
        execute_claimed_job(claim_next_job("teamcenter-worker"), resolve_job_executor)

        job = Job.objects.get()
        self.assertEqual(job.status, JobStatus.SUCCEEDED)
        self.assertEqual(job.result_summary["updated_objects"], 1)
        execute_client.assert_called_once()

    @patch(
        "integrations.teamcenter.job_executor.execute_with_client",
        side_effect=TeamcenterConnectionError("connection lost"),
    )
    def test_connection_loss_after_write_requires_reconciliation(self, _execute_client):
        self.client.post(
            reverse("teamcenter_property_update_job"),
            self.update_payload(),
            format="json",
            HTTP_IDEMPOTENCY_KEY="teamcenter-write-2",
        )
            ...
            "updates": [
                {
                    ...
                    "properties": {"object_name": ["Controlled value"]},
                }
            ]
        }
