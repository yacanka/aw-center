...
def required_extra_fields(field_map):
    return [
        field
        for identifier, field in field_map.items()
        if identifier not in BASE_FIELDS
...
def unsupported_fields(field_map, required):
    blockers = [safe_identity(field_id, field_id) for field_id in REQUIRED_SCREEN_FIELDS - field_map.keys()]
    blockers.extend(
        safe_identity(field.get("id"), field.get("name"))
        for field in required
        if not field_supported(field)
    )
...
def missing_fields(values, fields):
    return [
        safe_identity(field["id"], field.get("name"))
        for field in fields
        if not has_value(values.get(field["id"]))
    ]
...
def stale_value_warnings(values, field_map):
    ...
    return [
        f"Saved field {identifier} is no longer on the JIRA create screen and will be ignored."
