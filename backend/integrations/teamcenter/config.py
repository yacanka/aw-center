"""Validated Teamcenter adapter configuration."""
...
class TeamcenterClientConfig:
    ...
    def __post_init__(self) -> None:
        try:
            self.base_url = normalize_outbound_base_url(self.base_url)
        except ValueError as error:
            raise TeamcenterConfigurationError(
                "TEAMCENTER_BASE_URL must be a valid HTTP(S) origin without credentials or query data."
            ) from error
