...
def validate_check_ids(values):
    ...
    if not isinstance(requested, list) or not requested:
        raise JobExecutionFailure("Select at least one analysis check.", "WORD_ANALYZER_CHECKS_INVALID")
    normalized = list(dict.fromkeys(str(value) for value in requested))
    if len(normalized) > len(ANALYSIS_CHECKS) or any(value not in ANALYSIS_CHECKS for value in normalized):
        raise JobExecutionFailure("Unsupported analysis check selected.", "WORD_ANALYZER_CHECKS_INVALID")
    return normalized
