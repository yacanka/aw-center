...
from compliance.attention import compliance_attention_items
...
from .decisions import filter_user_decisions
...
def available_items(user):
    ...
    items.extend(import_items(user))
    if can_manage_invitations(user):
        items.extend(invitation_items())
    items.extend(compliance_attention_items(user, attention_item, MAX_SOURCE_ITEMS))
    return items
...
def jira_draft_items(user):
    ...
    drafts = (
        ...
        .annotate(
            ...
            authorized_project_count=Count(
                "projects",
                filter=Q(projects__id__in=project_ids, projects__enabled=True),
                distinct=True,
            ),
        )
        .filter(
            project_count__gt=0,
            project_count=F("authorized_project_count"),
        )
        .order_by("-updated_at")[:MAX_SOURCE_ITEMS]
    )
...
def import_items(user):
    ...
    audits = ImportAudit.objects.filter(
        project_id__in=project_ids,
        status__in=[ImportAudit.Status.FAILED, ImportAudit.Status.PARTIAL],
        started_at__gte=recent_boundary(),
    ).select_related("project").order_by("-started_at")[:MAX_SOURCE_ITEMS]
...
def import_item(audit):
    ...
    failed = audit.status == ImportAudit.Status.FAILED
    ...
    return attention_item(
        ...
        action_path=f"/compdocs/{audit.project.slug}?audit={audit.pk}",
