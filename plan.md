...
42. **Tamamlandı - Uygulama içi job takibi:** Medya dönüştürme, Word çeviri, belge analizi ve cover-page üretimi artık queue sonrasında Job Center'a zorunlu yönlendirme yapmıyor. İşi başlatan ekran URL ile geri yüklenebilen canlı ilerleme, mesaj, iptal, retry, indirme ve açık Job Center geçişi sunuyor; DCC üretimi de aynı iptal kontrolüne sahip.
43. **Tamamlandı - Kullanıcıya özel analiz checklist'i:** Kullanıcıların varsayılan kontroller yanında profil-bağlı özel sorular kaydetmesi, silmesi ve belge analizinde seçmesi; owner doğrulaması, sınırlı girdi, dedicated JSON profil alanı ve job oluşturulurken değişmez soru snapshot'ı ile eklendi.
44. **Tamamlandı - CompDoc satır çalışma alanı:** Kaybolan/yoğun aksiyon sütunu kaldırıldı; doküman satırı çift tıklama veya klavye Enter ile yetki-duyarlı sağ çalışma alanını açıyor. Kimlik, durum, gereksinim, not ve mevcut detay/düzenleme/export/path/silme işlemleri tek kurumsal bağlamda toplandı.
...
5. **Tamamlandı - Düzeltilebilir veri kalitesi:** Dashboard kalite ve operasyon uyarıları ilgili server-side filtreli CompDoc listesine deep-link veriyor; Panel/ATA ve cover-page sinyalleri düzeltilecek kayıtları açıyor.
6. **P2 - Kayıtlı dashboard görünümleri:** Kullanıcıya özel panel/status/tarih filtreleri, görünüm kaydetme ve paylaşılabilir güvenli URL parametreleri ekle.
...
5. **Tamamlandı - Dashboard Excel round-trip:** `Compliance Documents` ilk sayfa ve import sözleşmesinin kaynağı olarak korunurken export açılışı formül tabanlı KPI, durum ve panel grafikleri içeren `Dashboard` sayfasına taşındı. Excel onarımına yol açan çakışan worksheet/table AutoFilter tanımı kaldırıldı; stil, dondurulmuş başlık, veri doğrulama, formül enjeksiyonu koruması ve kayıpsız status history uzlaştırması korunuyor.
...
8. **P2 - Staging kabulü:** Gerçek DocProof fixture/hesabı ve seçilen Outlook veya SMTP transportuyla TLS, teslim edilebilirlik, bounce davranışı ve HTML istemci uyumluluğunu doğrula; Windows/Outlook oturumunda oluşturulan `.msg` dosyasını da gerçek istemcide açma-düzenleme-gönderme akışıyla kabul et.
...
## 77. Compliance Documents lifecycle and work management roadmap

1. **Tamamlandı - Güvenli kayıt temeli:** Create/import yetkileri ayrıldı; koleksiyon silme kapatıldı; nullable arşiv, sahip, ekip ve sonraki aksiyon alanları sekiz proje ile history tablolarına additive migration olarak eklendi.
2. **Tamamlandı - Veri bütünlüğü:** Zorunlu ve bounded cover-page, metin ve JSON-list doğrulaması; kontrol karakteri temizliği, duplicate normalizasyonu ve aynı proje içi Panel/ATA eşleşmesi uygulandı. Anonim migration preflight komutu eklendi.
3. **Tamamlandı - Immutable lifecycle ledger:** Legacy `status_flow` korunarak deterministik batch backfill yapıldı; yeni geçişler aktör, gerekçe, tarih, kaynak ve optimistic sürümle atomik event ledger’a ve legacy projeksiyona yazılıyor.
4. **Tamamlandı - Birleşik activity:** Hassas path değerlerini maskeleyen, alan farklarını, lifecycle olaylarını, ownership/arşiv hareketlerini ve review/approval kararlarını bounded timeline’da birleştiren endpoint ve workspace görünümü eklendi.
5. **Tamamlandı - İş sahipliği ve karar görevleri:** Proje görünürlüğü olan aktif AW Center kullanıcı/grup araması, owner/team/deadline yönetimi ve lifecycle’dan bağımsız review/approval atama ve karar kayıtları eklendi.
6. **Tamamlandı - Günlük operasyon:** Mine, My Team, Unassigned, Due Soon, Overdue, Pending Review, Pending Approval ve Archived filtreleri; en fazla 100 sürümlü kayıt için atomik bulk work/transition/archive/restore ve seçili Excel export eklendi.
7. **Tamamlandı - Operasyonel görünürlük:** Action Center owner/reviewer terminlerini kişiye özel ve güvenli deep-link ile gösteriyor; dashboard operasyon sayaçları ve veri kalitesi kartları ilgili server-side filtrelere bağlanıyor.
8. **Tamamlandı - Erişilebilir responsive UI:** Tek tıklama/touch ve Enter/Space workspace, odaklanabilir/isimli toolbar aksiyonları, kaydedilmemiş değişiklik koruması, alan hataları ve 409 karşılaştırma akışı eklendi. Form Identity, References, Ownership, Workflow, Notes ve History bileşenlerine ayrıldı.
9. **P2 - Hesap-bağlı saved views:** Lokal tablo tercihlerini isimlendirilebilir, cihazlar arası senkronize ve güvenli paylaşılabilir kullanıcı görünümlerine taşı.
10. **P2 - Tarihsel KPI ve risk kalibrasyonu:** Günlük snapshotlar, approval lead time ve gerçek üretim geri bildirimiyle sürümlü risk/SLA politikası oluştur.
11. **P1 - Gerçek transport staging kabulü:** DocProof ve seçilen Outlook/SMTP transportunu gerçek fixture/hesap, TLS, teslim edilebilirlik ve istemci uyumluluğuyla staging ortamında doğrula.
