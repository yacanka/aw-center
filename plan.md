...
31. **Tamamlandı - Kalıcı JIRA → DCC DOCX üretimi:** Process-local DCC cache/SSE ve base64 indirme kaldırıldı; geçici JIRA session yalnız snapshot yakalama isteğinde ve tüm DCC araçlarında yalnız bellek içinde kullanılıyor. Sürümlü ve boyut sınırlı snapshot özel job girdisi olarak SHA-256 bütünlüğüyle saklanıyor, owner-scoped worker gerçek OOXML çıktısını doğruluyor, kesinti/retry/audit/indirme Job Center üzerinden yönetiliyor ve sıfır-alt görev ile çelişkili Responsible AS sınırları açıkça ele alınıyor.
...
35. **Tamamlandı - Job worker süreç sürekliliği:** Ana launcher'ın dev/prod modları ve küçük geliştirme starter'ı web süreçleriyle birlikte kalıcı worker'ı da başlatıp denetliyor; DCC onayının gerçek dry-render, queue, worker ve doğrulanmış DOCX tamamlanma zinciri tek regresyon testinde korunuyor. Job Center yalnız gerçekten `awaiting_confirmation` olan DCC preview'larında onay yönlendirmesi gösteriyor.
...
37. **Tamamlandı - CompDoc -> DCC izlenebilirlik köprüsü:** Yetkili kullanıcı en fazla 50 proje kaydını server-paginated tablodan seçip DCC Creator'a aktarabiliyor. Backend JIRA projesi, UUID'ler, kayıt varlığı ve `view_compdoc` yetkisini capture/replay/confirmation aşamalarında doğruluyor; son history sürümleri canonical SHA-256 fingerprint ile immutable snapshot'a ve doğrulanmış DOCX register ekine bağlanıyor.
38. **Tamamlandı - Kalıcı ters CompDoc -> DCC audit'i:** Yalnız açık confirmation sırasında oluşan bağımsız provenance kayıtları kaynak history/fingerprint bilgisini job retention'dan sonra da koruyor. CompDoc detayı güncel/eski kaynak sürümünü, en yeni retry durumunu ve yalnız sahibine açılan Job Center bağlantısını iki ayrı read permission ile server-paginated gösteriyor.
39. **Tamamlandı - Açıklanabilir DCC readiness ve risk acknowledgment:** Dry-render önizlemesi template, JIRA alanları, panel kapsamı ve CompDoc teknik referans/otorite olgunluğunu ağırlıklı skor ve eyleme dönük checklist olarak gösteriyor. Warning'lerin tam kod kümesi backend'de açık kullanıcı onayı olmadan queue'ya geçemiyor ve kabul kararı immutable Job event'inde saklanıyor.
...
41. **Tamamlandı - Açıklanabilir CompDoc önerileri:** DCC preview, captured JIRA alanları/panel başlıkları içindeki teknik referans, cover page, ATA, panel ve anlamlı doküman adı kanıtlarından yetki kontrollü öneriler üretiyor. Kullanıcının seçtiği kayıtlar otomatik bağlanmadan aynı doğrulanmış JIRA snapshot'ından yeni immutable preview'a aktarılıyor; source preview atomik olarak superseded ediliyor, lineage ve karar olayları audit'te korunuyor.
42. **Tamamlandı - CompDoc kaynak değişikliği riskleri:** Son 14 günde değişen ve en yeni onaylı DCC izi güncel history sürümünün gerisinde kalan CompDoc'lar, hem DCC hem proje okuma yetkisi gerektiren Action Center uyarılarına dönüştürülüyor. Uyarı doğrudan ilgili kaydı ve ters DCC geçmişini açıyor; kararlar tek history sürümüne bağlandığı için sonraki değişiklik yeniden görünür oluyor ve güncel kaynakla yeni confirmation riski otomatik çözüyor.
43. **Tamamlandı - Çakışmasız CompDoc düzenleme ve DCC etki kapısı:** Liste/detay yanıtları sunucu türetilmiş güncel history sürümünü taşıyor; manuel PUT/PATCH bu sürümü transaction ve row lock altında zorunlu doğrulayıp eski yazımları hiçbir veri değiştirmeden 409 ile reddediyor. Yetkili editör, bağlantılı DCC trace sayısını ve issue anahtarlarını düzenleme sırasında görüyor; etki yüklenmeden veya bağlantılı DCC etkisi açıkça kabul edilmeden Update etkinleşmiyor, başarılı değişiklik aktör audit'i ve yeni sürümle dönüyor.
44. **Tamamlandı - Alan bazlı DCC kaynak etkisi:** Ters trace API'si güncel CompDoc ile DCC'ye gerçekten render edilen historical alanları tek bulk sorguyla karşılaştırıp yalnız alan kodu/etiketi/kategorisini döndürüyor. Action Center not/path gibi DCC çıktısına girmeyen history değişikliklerini bastırıyor; gerçek referans, workflow, kimlik, sınıflandırma ve sahiplik değişikliklerini değer sızdırmadan açıklıyor. CompDoc geçmişi güncel, etkisiz metadata değişikliği, DCC-visible değişiklik ve bulunamayan history durumlarını ayrı gösteriyor.
45. **Tamamlandı - Credential-free güncel CompDoc DCC revizyonu:** Eski DCC trace'inin bütünlüğü doğrulanmış özel JIRA snapshot'ı yeniden kullanılarak tüm bağlı CompDoc'ların güncel sürümleriyle tek tıkla yeni, owner-bound ve onay gerektiren önizleme üretilebiliyor. Kaynak çıktı değişmiyor; permission, lineage, retention, idempotency ve TOCTOU kontrolleri fail-closed çalışıyor, DCC-visible değişiklik confirmation öncesi yeniden doğrulanıyor.
